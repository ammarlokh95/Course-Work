...
def computefaces(corner1, corner2, vanishingpt, imwidth, imheight, focallen):
    ...
    h = imheight - vanishingpt[1] # might be vanishgpt[1]
    ...
    width2d =  abs(backleft - backright)
    height2d =  abs(backbottom - backtop)
    backwidth =  (backheight * width2d) / height2d
    ...
    top = backdepth - (b * (backdepth / vanishingpt[1]))
    bottom = backdepth - (a * (backdepth / h))
    left = backdepth - ((vanishingpt[0] - backleft) * (backdepth / (vanishingpt[0])))
    right = backdepth - ((backright - vanishingpt[0]) * (backdepth / (imwidth - vanishingpt[0])))
    ...
    right3d = np.array([
        [backwidth, backheight, right],
        [backwidth, backheight, 0],
        [backwidth, 0, 0],
        [backwidth, 0, right]
    ])
    top3d = np.array([
        [backwidth, backheight, top],
        [0, backheight, top],
        [0, backheight, 0],
        [backwidth, backheight, 0]
    ])
...
def computehomography(facefrom, faceto):
    ...
    A[0] = [facefrom[0][1], facefrom[0][0], 1, 0, 0, 0, -faceto[0][1] * facefrom[0][1], -faceto[0][1] * facefrom[0][0], -faceto[0][1]]
    A[1] = [0, 0, 0, facefrom[0][1], facefrom[0][0], 1, -faceto[0][0] * facefrom[0][1], -faceto[0][0] * facefrom[0][0], -faceto[0][0]]  
    A[2] = [facefrom[1][1], facefrom[1][0], 1, 0, 0, 0, -faceto[1][1] * facefrom[1][1], -faceto[1][1] * facefrom[1][0], -faceto[1][1]]
    A[3] = [0, 0, 0, facefrom[1][1], facefrom[1][0], 1, -faceto[1][0] * facefrom[1][1], -faceto[1][0] * facefrom[1][0], -faceto[1][0]]
    A[4] = [facefrom[2][1], facefrom[2][0], 1, 0, 0, 0, -faceto[2][1] * facefrom[2][1], -faceto[2][1] * facefrom[2][0], -faceto[2][1]]
    A[5] = [0, 0, 0, facefrom[2][1], facefrom[2][0], 1, -faceto[2][0] * facefrom[2][1], -faceto[2][0] * facefrom[2][0], -faceto[2][0]]
    A[6] = [facefrom[3][1], facefrom[3][0], 1, 0, 0, 0, -faceto[3][1] * facefrom[3][1], -faceto[3][1] * facefrom[3][0], -faceto[3][1]]
    A[7] = [0, 0, 0, facefrom[3][1], facefrom[3][0], 1, -faceto[3][0] * facefrom[3][1], -faceto[3][0] * facefrom[3][0], -faceto[3][0]]
...
def homographywarp(source, homography, imwidth, imheight):
    ...
    for y in xrange(imheight):
        for x in xrange(imwidth):
            ...
            p = np.array([x,y,1])
            hp = np.linalg.solve(homography, p)
            # Make it the proper scale by dividing by w
            normalizedp = (hp/hp[2][0])
            # Set the pixel color in newimage. be sure to check your bounds.
            newimage[y][x] = source[normalizedp[0][0].type(int)][normalizedp[1][0].type(int)]
