...
regExpToNfa (Cat r s) =
	Nfa (r_qs `union` s_qs `union` fromList [q0,qf])
		 (r_moves `union` s_moves `union` fromList catMoves)
		 q0
		 (singleton qf)
	where
		-- NFA correspounding to reg exp r, suitable renumbered.
		r_nfa@(Nfa r_qs r_moves r_start _) =
			numberNfaFrom 1 $ regExpToNfa r
		-- NFA correspounding to the reg exp. s, suitable renumbered.
		s_nfa@(Nfa s_qs s_moves s_start _) =
			numberNfaFrom (nfaSize r_nfa + 1) $ regExpToNfa s
		q0 = 0
		qf = nfaSize r_nfa + nfaSize s_nfa + 1
		catMoves = [ Emove q0 r_start
					  , Emove (acceptState r_nfa) s_start
